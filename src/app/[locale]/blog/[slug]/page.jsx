...
export async function generateStaticParams() {
  const paths = [];
  Object.entries(SLUG_MAPPINGS.en).forEach(([enSlug, translations]) => {
    Object.entries(translations).forEach(([locale, translatedSlug]) => {
      if (VALID_LOCALES.includes(locale)) {
        paths.push({ locale, slug: translatedSlug });
        paths.push({ locale, slug: enSlug }); // Also allow English slugs for each locale
      }
    });
  });
  return paths;
...
// Generate metadata using only localization data
